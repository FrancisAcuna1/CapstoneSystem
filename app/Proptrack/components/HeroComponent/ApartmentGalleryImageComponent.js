...
import { Box, Typography, Grid, Button, IconButton, Container, ImageList, ImageListItem, ImageListItemBar, styled, Skeleton} from '@mui/material';
...
export default function ApartmentGalleryComponent({loading, setLoading, apartmentId, propsId}) {
  ...
  useEffect(() => {
      const fetchDetails = async () => {
        setLoading(true);
          try{
              const response = await fetch(`http://127.0.0.1:8000/api/apartmentdetails/${propsId}/${apartmentId}`, {
                  method: 'GET',
                  headers: {
