...
const fetcherBarangays = async ([url, token]) => {
...
export default function EditTenantModal({
  ...
  const {data: resposneBarangay, error: errorBarangay} = useSWR(
    token && municipalityCode ? [`https://psgc.gitlab.io/api/cities-municipalities/${municipalityCode}/barangays`, token] : null,
    fetcherBarangays, {
  ...
  console.log(errorBarangay);
  console.log(resposneBarangay);
  useEffect(() => {
    if (resposneBarangay) {
      setAllBarangays(resposneBarangay);
    }
  }, [resposneBarangay])
  ...
  useEffect(() => {
    if (response) {
      ...
      const selectedMunicipality = SORSOGON_MUNICIPALITIES.find(
        (m) => m.name === tenantData?.municipality
      );
      setFormData({
  ...
  const uniqueBarangays = Object.values(
    allBarangays.reduce((acc, current) => {
      ...
      if (!acc[current.name]) {
        ...
        if (
          !current.oldName ||
          (current.oldName && !acc[current.name].oldName)
        ) {
          acc[current.name] = current;
        }
      }
                ...
                <Grid item xs={12} sm={4} sx={{ mt: -1 }}>
                  ...
                  >
                    <Autocomplete
                      ...
                      value={
                        allBarangays.find(
                          (mun) => mun.name === formData.barangay
                        ) || null
                      }
                ...
                <Grid item xs={12} sm={4} sx={{ mt: 1 }}>
                  ...
                  >
                    <Autocomplete
                      ...
                      getOptionLabel={(option) => option.name || ""}
                      value={
                        SORSOGON_MUNICIPALITIES.find(
                          (mun) => mun.name === formData.municipality
                        ) || null
                      }
                      onChange={(event, newValue) => {
