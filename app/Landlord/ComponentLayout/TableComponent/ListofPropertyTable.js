...
export default function UnitListTable({
  ...
  const [selectedDeleteProperty, setSelectedDeleteProperty] = useState({
    id: null,
    type: "",
  });
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (selectedCategory === "All") {
              ...
              setPropertyData({
                apartments: data?.status || [],
                boarding_houses: data?.[0] || [],
              });
  ...
  const handleDelete = async () => {
    ...
    if (accessToken) {
      ...
      for (const { id, property_type } of selectedData) {
        ...
        if (!property) {
          console.log(
            `Property with ID ${id} and Type ${property_type} not found`
          );
          continue;
        }
  ...
  const getRoomsForProperty = (propertyId) => {
    return (
      propertyData.boarding_houses.find((bh) => bh.id === propertyId)?.rooms ||
      []
    );
  };
  ...
  const handleClick = (
    ...
    if (status === "Available") {
      ...
      if (property_type === "Apartment") {
        ...
        setLoading(true);
        router.push(
          `/Landlord/Property/${propsid}/boardinghouse/${boardinghouseId}`
        );
      }
      ...
      if (property_type === "Apartment") {
        router.push(
          `/Landlord/Property/${propsid}/occupiedapartment/${occupiedid}`
        );
      } else if (property_type === "Boarding House") {
        router.push(
          `/Landlord/Property/${propsid}/occupiedboardinghouse/${occupiedboardinghouseId}`
        );
      }
    }
  };
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    const allProperties = [
      ...propertyData.apartments.map((item) => ({
        ...item,
        uniqueKey: `${item.id}-apartment`,
      })),
      ...propertyData.boarding_houses.map((item) => ({
        ...item,
        uniqueKey: `${item.id}-boardinghouse`,
      })),
    ];
    ...
    <Box sx={{ maxWidth: 1400, margin: "auto", overflowX: "auto" }}>
      ...
      >
        ...
        >
          ...
          {selectedItem.length > 0 ? (
            <>
              {selectedItem.some((selectedId) =>
                // displayedData.some((item) =>
                //     item.uniqueKey === selectedId && item.status === "Occupied"
                // )
                displayedData.some(item => {
                  // Check if status is 'Occupied' and if there are still available beds
                  const isFullyOccupied = item.status === 'Occupied';
              ...
              <TableRow>
                <StyledTableCell>
                  <Checkbox
                    ...
                    indeterminate={
                      selectedItem.length > 0 &&
                      selectedItem.length < propertyData.length
                    }
                    inputProps={{
                ...
                >
                  # of Rooms{" "}
                  {sortConfig.key === "number_of_rooms" &&
                    (sortConfig.direction === "asc" ? (
                      <NorthIcon
                        fontSize="extrasmall"
                        justifyContent="center"
                ...
                <StyledTableCell sx={{ width: "8%" }}>
                  Max # of Occupants per room
                  {sortConfig.key === "number_of_beds" &&
                    (sortConfig.direction === "asc" ? (
                      <NorthIcon
                        fontSize="extrasmall"
                        justifyContent="center"
                ...
                displayedData.map((property, index) => {
                  ...
                  return (
                    <StyledTableRow
                      ...
                      onChange={(event) =>
                        handleCheckBoxChange(event, property.uniqueKey)
                      }
                    >
                      <TableCell>
                        <Checkbox
                          ...
                          : (() => {
                              ...
                              return rooms.length > 0
                                ? rooms.map((room) => {
                                    ...
                                    const availableBedsInfo = availableBeds
                                      .map((bed) => `Bed ${bed.bed_number}`)
                                      .join(", ");
                                    const occupiedBedsInfo = occupiedBeds
                                      .map((bed) => `Bed ${bed.bed_number}`)
                                      .join(", ");
                                    const tooltipTitle =
                      ...
                      <TableCell align="center">
                        ...
                        <AcceptToolTip title="Edit">
                          <IconButton
                            ...
                            onClick={() =>
                              handleEdit(property.id, property.property_type)
                            }
                          >
      ...
      <AlertDialog handleClose={handleCloseAlert} open={Dialongopen} />
