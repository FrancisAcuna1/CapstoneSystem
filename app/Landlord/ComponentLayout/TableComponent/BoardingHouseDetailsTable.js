...
const boardingHouses  = [
    {
        id: '1',
        boardingHouseId: 'BH001',
        name: 'Boarding House no.1',
        location: 'Peralta st., Brgy. Burabod, Sorsogon City',
        status: 'Occupied',
        inclusion: 'Kitchen, Stove, Sala set, Comfort Room',
        totalCapacity: 12,
        currentOccupants: 8,
        rooms: [
          { roomNumber: 1, beds: 4, occupied: 3 },
          { roomNumber: 2, beds: 4, occupied: 2 },
          { roomNumber: 3, beds: 2, occupied: 2 },
          { roomNumber: 4, beds: 2, occupied: 1 },
        ],
        amenities: ['Free Wi-Fi', 'Laundry Area', 'Study Room', 'CCTV Security'],
        monthlyRate: 5000,
    },
];

export default function BoardinghouseDetailsTable({params}){
...
//   };
  ...
  const sortedUnits = [...boardingHouses].sort((a, b) => {
  ...
  const handleSelectAllClick = (event) => {
      if (event.target.checked) {
        const newSelected = boardingHouses.map((n) => n.id);
  ...
  const handleExportToExcel = () => {
      const ws = XLSX.utils.json_to_sheet(boardingHouses);
            ...
            <TableHead sx={{backgroundColor: 'whitesmoke', p:1}}>
                ...
                <StyledTableCell> 
                  <Checkbox
                      ...
                      indeterminate={selectedItem.length > 0 && selectedItem.length < boardingHouses.length}
                      inputProps={{
                ...
                <StyledTableCell onClick={() => handleSort('name')}  >
                    Name {sortConfig.key === 'name' && (sortConfig.direction === 'asc' ? <NorthIcon   fontSize='extrasmall' justifyContent="center" color="#bdbdbd"/> : <SouthIcon  fontSize='extrasmall'/>)}
                </StyledTableCell>
                ...
                <StyledTableCell onClick={() => handleSort('propertype')} sx={{width: '8%'}}>
                  Monthly Rate {sortConfig.key === 'propertype' && (sortConfig.direction === 'asc' ? <NorthIcon   fontSize='extrasmall' justifyContent="center" color="#bdbdbd"/> : <SouthIcon  fontSize='extrasmall'/>)}
                </StyledTableCell>
                <StyledTableCell onClick={() => handleSort('propertype')} sx={{width: '8%'}}>
                  Current # of Occupants {sortConfig.key === 'propertype' && (sortConfig.direction === 'asc' ? <NorthIcon   fontSize='extrasmall' justifyContent="center" color="#bdbdbd"/> : <SouthIcon  fontSize='extrasmall'/>)}
                </StyledTableCell>
                <StyledTableCell onClick={() => handleSort('propertype')} sx={{width: '12%'}}>
                  Amenities {sortConfig.key === 'propertype' && (sortConfig.direction === 'asc' ? <NorthIcon   fontSize='extrasmall' justifyContent="center" color="#bdbdbd"/> : <SouthIcon  fontSize='extrasmall'/>)}
                </StyledTableCell>
               
                <StyledTableCell onClick={() => handleSort('propertype')} sx={{width: '8%'}}>
                  Total Capacity {sortConfig.key === 'propertype' && (sortConfig.direction === 'asc' ? <NorthIcon   fontSize='extrasmall' justifyContent="center" color="#bdbdbd"/> : <SouthIcon  fontSize='extrasmall'/>)}
                </StyledTableCell>
              ...
              {paginatedUnits.map((unit, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell>{unit.monthlyRate}</TableCell>
                    <TableCell>{unit.currentOccupants}</TableCell>
                    <TableCell>{unit.amenities.join(', ')}</TableCell>
                    <TableCell>{unit.totalCapacity}</TableCell>
